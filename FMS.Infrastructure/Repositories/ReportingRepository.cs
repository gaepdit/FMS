...
namespace FMS.Infrastructure.Repositories
{
    public class ReportingRepository : IReportingRepository
    {
        ...
        public async Task<IReadOnlyList<DelistedReportByDateRangeDto>> GetDelistedByDateRangeAsync(DateOnly? startDate,
            DateOnly? endDate)
        {
            var facilityList = await _context.Facilities.AsNoTracking()
                .Include(e => e.County)
                ...
                .Where(e => e.HsrpFacilityProperties.DateDeListed != null &&
                            ...
                            e.HsrpFacilityProperties.DateDeListed <= endDate.GetValueOrDefault())
        ...
        public async Task<IReadOnlyList<ListedReportByDateRangeDto>> GetListedByDateRangeAsync(DateOnly? startDate,
            DateOnly? endDate)
        {
            var facilityList = await _context.Facilities.AsNoTracking()
                .Include(e => e.County)
                ...
                .Where(e => e.FacilityStatus.Status == "Active")
                .Where(e => e.HsrpFacilityProperties.DateListed != null &&
                            e.HsrpFacilityProperties.DateListed >= startDate.GetValueOrDefault() &&
                            e.HsrpFacilityProperties.DateListed <= endDate.GetValueOrDefault())
        ...
        public async Task<IReadOnlyList<PAFReportRawDto>> GetPAFReportAsync()
        {
            ...
            return (await conn.QueryAsync<PAFReportRawDto>("dbo.PAF_Report", commandType: CommandType.StoredProcedure))
                .ToList();
        }
        ...
        public async Task<IReadOnlyList<AbndInacStatusTrackerDto>> GetAbndInacStatusTrackerReportAsync()
        {
            var facilityList = await _context.Facilities.AsNoTracking()
                ...
                .Select(e => new AbndInacStatusTrackerDto()
                {
                    ...
                    GAPSAssessment = e.StatusDetails.GAPSAssessment != null
                        ? e.StatusDetails.GAPSAssessment.Name
                        : "None",
                    CostEstimate = e.StatusDetails.CostEstimate,
                    AbndInacInfo = e.StatusDetails.AbandonedInactive != null
                        ? e.StatusDetails.AbandonedInactive.Name
                        : "None",
