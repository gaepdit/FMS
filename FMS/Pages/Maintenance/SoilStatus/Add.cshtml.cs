using System.Threading.Tasks;
using FMS.Domain.Dto;
using FMS.Domain.Entities.Users;
using FMS.Domain.Repositories;
using FMS.Platform.Extensions;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.Mvc.RazorPages;

namespace FMS.Pages.Maintenance.SoilStatus
{
    [Authorize(Roles = UserRoles.SiteMaintenance)]
    public class AddModel : PageModel
    {
        private readonly ISoilStatusRepository _repository;
        public AddModel(ISoilStatusRepository repository) => _repository = repository;

        [BindProperty]
        public SoilStatusCreateDto SoilStatus { get; set; }

        public void OnGet()
        {
            // Method intentionally left empty.
        }

        public async Task<IActionResult> OnPostAsync()
        {
            if (!ModelState.IsValid)
            {
                return Page();
            }

            SoilStatus.TrimAll();

            if (await _repository.SoilStatusNameExistsAsync(SoilStatus.Name))
            {
                ModelState.AddModelError("SoilStatus.Name", "Name entered already exists.");
            }

            if (!ModelState.IsValid)
            {
                return Page();
            }

            await _repository.CreateSoilStatusAsync(SoilStatus);

            TempData?.SetDisplayMessage(Context.Success,
                $"{MaintenanceOptions.SoilStatus} \"{SoilStatus.Name}\" successfully created.");

            return RedirectToPage("./Index", "select",
                new { MaintenanceSelection = MaintenanceOptions.SoilStatus });
        }
    }
}
